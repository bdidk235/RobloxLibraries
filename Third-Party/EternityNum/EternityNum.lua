...
function EN.toString(Value: EN): string
	...
	if Value.Layer ~= 0 then return Value.Layer * Value.Sign .. ";" .. Value.Exp end
...
function EN.abs(Value): EN
	...
	if Value.Sign == 0 then return ZERO end
...
function EN.recip(Value): EN
	...
	if Value.Layer == 0 then return EN.new(Value.Sign, 0, 1 / Value.Exp) end
...
function baseLog(Value, Base): EN
	...
	if EN.IsNaN(Base) or EN.IsNaN(Value) then return NaN end
...
function EN.log10(Value): EN --// log10(x)
	...
	if Value.Layer > 0 then return EN.new(math.sign(Value.Exp), Value.Layer - 1, math.abs(Value.Exp)) end
...
function EN.add(Value, Value2): EN
	...
	if Value.Sign == -Value2.Sign and Value.Layer == Value2.Layer and Value.Exp == Value2.Exp then return ZERO end

	local a, b

	if Value.Layer >= 2 or Value2.Layer >= 2 then return EN.maxAbs(Value, Value2) end
...
function EN.toScientific(Value: EN): string
	if Value.Layer > 2 then
		if AllowOverflow then return "" end
		return "Inf"
	end
...
function EN.mul(Value, Value2): EN
	...
	if Value.Layer == Value2.Layer and Value.Exp == -Value2.Exp then return EN.new(Value.Sign * Value2.Sign, 0, 1) end
	...
	if a.Layer == 0 and b.Layer == 0 then return EN.fromNumber(a.Sign * b.Sign * a.Exp * b.Exp) end
	if a.Layer >= 3 or (a.Layer - b.Layer >= 2) then return EN.new(a.Sign * b.Sign, a.Layer, a.Exp) end
	if a.Layer == 1 and b.Layer == 0 then return EN.new(a.Sign * b.Sign, 1, a.Exp + math.log10(b.Exp)) end
	if a.Layer == 1 and b.Layer == 1 then return EN.new(a.Sign * b.Sign, 1, a.Exp + b.Exp) end
...
function EN.pow10(Value): EN
	...
	if Value.Sign > 0 and Value.Exp > 0 then return EN.new(Value.Sign, Value.Layer + 1, Value.Exp) end
	if Value.Sign < 0 and Value.Exp > 0 then return EN.new(-Value.Sign, Value.Layer + 1, -Value.Exp) end
...
function EN.toSuffix(Value: EN, Digits: number?): string
	...
	if Exponent < 3 then return CutDigits(Mantissa * 10 ^ Modulus3, Digits) .. Sets[Exponent + 1] end
...
function EN.toLayerNotation(Value, Digits: number?): string
	...
	if EN.between(Value, ZERO, ONE) then return "1 / " .. EN.short(EN.div(ONE, Value)) end
	if EN.between(EN.neg(Value), ZERO, ONE) then return "-1 / " .. EN.short(EN.div(ONE, EN.neg(Value))) end

	if Value.Sign == 0 then return "E(0)0" end
	local LayerSign = if Value.Sign < 0 then "-" else ""

	if Value.Exp < 0 then
		return LayerSign .. "E(" .. Value.Layer .. "-" .. ")" .. CutDigits(math.abs(Value.Exp), Digits)
	end

	return LayerSign .. "E(" .. Value.Layer .. ")" .. CutDigits(Value.Exp, Digits)
end

function EN.short(Value, Digits): string
	Value = EN.convert(Value)
	if EN.le(Value, SuffixLimit) then return EN.toSuffix(Value, Digits) end
...
function EN.gamma(Value): EN
	...
	if Value.Layer == 0 then
		if EN.le(Value, { 1, 0, 24 }) then return EN.fromNumber(F_Gamma(Value.Sign * Value.Exp)) end
...
function EN.exporand(min, max): EN
	min, max = EN.convert(min), EN.convert(max)
	local sign, sign2 = min.Sign, max.Sign
	min = EN.mul(EN.exp(EN.abs(min)), sign)
	max = EN.mul(EN.exp(EN.abs(max)), sign2)
